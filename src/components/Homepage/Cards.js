...
export const Cards = ({ user }) => {
    ...
    return (
            ...
            <div id="card" className="bg-white dark:bg-zinc-800 shadow-lg rounded-lg px-4 py-2 flex items-center justify-between m-2 my-2" >
                <Link to={`/history?userId=${user.id}`}>
                <div className="flex flex-col justify-center">
                    <span className="font-semibold text-zinc-800 dark:text-zinc-200">{user.name}</span>
                    <span className="text-zinc-600 dark:text-zinc-400 text-sm">{user.address}</span>
                </div>
                </Link>
